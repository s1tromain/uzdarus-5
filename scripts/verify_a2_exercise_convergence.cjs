#!/usr/bin/env node
/**
 * verify_a2_exercise_convergence.cjs — A2's exercise lifecycle, driven.
 *
 * A2 already stepped through its exercises under the shared session and
 ...
 * paid-courses/a2-course.html. A lifecycle that behaves in isolation proves
 ...
 * ================================================================ */
{
    ...
    ok(/draftFor\(/.test(H.read('a2-host.js')),
 ...
 * ================================================================ */
{
     ...
     *
     * Everything above drives a2FinishExercises directly, which proves the
     ...
     * the lifecycle, and must not be the old whole-topic claim. */
    {
        ...
        ok(dep.indexOf("return a2FinishExercises(id == null ? topicId : id, payload);") >= 0,
            C + ': and it routes into a2FinishExercises');
        eq(C + ': the dep does not claim the whole topic',
            /completeCourseTopic\(/.test(dep), false);
        ok(/function a2FinishExercises/.test(SRC),
