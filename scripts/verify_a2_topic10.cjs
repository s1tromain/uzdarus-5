...
/* ------------------------------- 6. it renders, grades and completes */
{
    ...
    w.__api.startTopicExercises(10);
    const D = w.document;
    ...
    const gram = require('./_grammar_material.cjs').materialOf('A2', 10);
    const lesson = gram.text;
    ok(/У меня болят ноги/.test(lesson), 'the grammar reaches the reader');
