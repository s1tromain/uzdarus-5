#!/usr/bin/env node
/**
 * verify_a2_topic14_viewport.cjs — Topic 14 must fit a phone, measured by a real
 * browser.
 *
...
const t6 = vm.runInNewContext('(' + S.slice(j, e + 1) + ')', {}).topics.find((t) => t.id === 14);
const groups = t6.topic14Exercises.exercises;
...
const tmp = path.join(os.tmpdir(), 'uz-t14-viewport.html');
...
VIEWPORTS.forEach(([w, h]) => {
    ...
    /* Every tappable control must clear the 44px touch target. This lesson is
       chip-heavy — ex3 alone puts four chips on every one of its ten rows. */
    ok(r.minBtnH >= 44,
...
console.log('='.repeat(60));
if (fail) {
    console.log(`  ❌ A2 TOPIC 14 VIEWPORT: ${fail} failed / ${pass + fail}\n`);
    failures.forEach((f, i2) => console.log(`   ${i2 + 1}. ${f}`));
    console.log('='.repeat(60) + '\n');
    process.exit(1);
}
console.log(`  ✅ A2 TOPIC 14 VIEWPORT: ${pass}/${pass} passed (measured in Chrome)`);
console.log('='.repeat(60) + '\n');
