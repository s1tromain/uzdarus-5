#!/usr/bin/env node
/**
 * verify_b2_topic15_viewport.cjs — Topic 15 must fit a phone, measured by a real
 * browser.
 *
...
const t15 = jw.B2_LESSON_DATA.topics.find((t) => t.id === 15);
ok(!!t15, 'topic 15 lesson data loaded');
if (!t15) { console.log('missing lesson 15'); process.exit(1); }
const groups = t15.exercises;
...
eq('every option chip rendered (ex1 3×10 + ex4 3×10 + ex5 2×10 + audio 2×10)', CHIPS, 100);
eq('all seventy writing inputs rendered (seven open input groups)', INPUTS, 70);
eq('the audio player rendered exactly once', (stepsHtml.match(/<audio/g) || []).length, 1);
{
    ...
    eq('the rendered src decodes to the topic 13 recording',
        decodeURIComponent(src).replace(/^\.\.\//, ''), 'audios/Б2 15 урок.mp3');
...
console.log(`  UI CSS ${uiCss.length}b · page CSS ${pageCss.length}b · session CSS ${sessionCss.length}b`
    + ` · chips ${CHIPS} · inputs ${INPUTS} · tables ${(t15.grammar.match(/class="b2g-t"/g) || []).length}`
...
<div class="vp-grammar">${t15.grammar}</div>
${stepsHtml}
<div class="uz-foot">
...
VIEWPORTS.forEach(([w, h]) => {
    ...
    /* grammar tables */
    eq(`${label}: all ten grammar tables rendered`, r.tables, 10);
    ...
    /* Правда / Ложь chips */
    eq(`${label}: every option chip rendered`, r.chips, 100);
    ...
    eq(`${label}: all seventy writing inputs rendered`, r.inputs, 70);
...
console.log('='.repeat(60));
if (fail) {
    console.log(`  ❌ B2 TOPIC 15 VIEWPORT: ${fail} failed / ${pass + fail}\n`);
    failures.forEach((f, i) => console.log(`   ${i + 1}. ${f}`));
    console.log('='.repeat(60) + '\n');
    process.exit(1);
}
console.log(`  ✅ B2 TOPIC 15 VIEWPORT: ${pass}/${pass} passed (measured in Chrome)`);
console.log('='.repeat(60) + '\n');
