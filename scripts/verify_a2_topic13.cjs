...
/* ------------------------------- 6. it renders, grades and completes */
{
    ...
    w.__api.startTopicExercises(13);
    const D = w.document;
    ...
    const gram = require('./_grammar_material.cjs').materialOf('A2', 13);
    const lesson = gram.text;
    ok(/Если бы у меня было время/.test(lesson), 'the grammar reaches the reader');
