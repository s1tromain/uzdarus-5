/**
 * _a1_page_harness.cjs — run the REAL A1 course page functions.
 *
 * Both A1 behavioural suites had the same blind spot: they proved a1-host.js
 ...
 * paid-courses/a1-course.html, not a copy — into a JSDOM window alongside the
 * real a1-host.js, the real session and the real UI, and lets the tests drive
...
const PAGE = fs.readFileSync(path.join(ROOT, 'paid-courses/a1-course.html'), 'utf8');
...
const PAGE_FNS = [
    'mountA1Practice', 'a1FinishExercises', 'a1ApplyOutcome', 'a1RetrySave',
    'a1ShowNotice', 'a1ShowSaveError', 'a1ExerciseCtaLabel', 'a1OpenReview',
    'a1RenderDone', 'a1RenderLegacy', 'a1RenderReview', 'a1SyncComponent'
];
...
function makePage(opts) {
    ...
    const dom = new JSDOM(
        ...
        { runScripts: 'outside-only', pretendToBeVisual: true, virtualConsole: vc,
          url: 'https://uzdarus.test/paid-courses/a1-course.html' });
    const w = dom.window;
    ...
    ['exercise-session.js', 'sentence-builder.js', 'course-exercise-ui.js', 'a1-host.js']
    ...
    const ctx = {
        ...
        a1CourseState: opts.courseState || null,
    ...
    const src = PAGE_FNS.map((n) => lift(n)).join('\n\n') +
        ...
        /* ACCESSORS, not values. The page REASSIGNS completedTopics and
           a1CourseState — reading a copy taken at construction time would
           show the tests the state before the server answered. */
        ', getCompletedTopics: function(){return completedTopics;}' +
        ', getCourseState: function(){return a1CourseState;}' +
        ', getPendingRetry: function(){return a1PendingRetry;}};';

    const api = new Function(
        'window', 'document', 'localStorage', 'courseData', 'quizSection',
        'userQuizResults', 'completedTopics', 'a1CourseState',
        'loadTopics', 'updateProgress', 'ctx',
        '"use strict"; var a1PendingRetry = null;\n' + src
    )(w, w.document, w.localStorage, ctx.courseData, ctx.quizSection,
      ctx.userQuizResults, ctx.completedTopics, ctx.a1CourseState,
