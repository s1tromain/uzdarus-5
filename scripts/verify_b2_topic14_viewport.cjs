#!/usr/bin/env node
/**
 * verify_b2_topic14_viewport.cjs — Topic 14 must fit a phone, measured by a real
 * browser.
 *
...
const t14 = jw.B2_LESSON_DATA.topics.find((t) => t.id === 14);
ok(!!t14, 'topic 14 lesson data loaded');
if (!t14) { console.log('missing lesson 14'); process.exit(1); }
const groups = t14.exercises;
...
eq('every option chip rendered (ex7 2×10 + ex10 5×10 + audio 2×10)', CHIPS, 90);
...
eq('the audio player rendered exactly once', (stepsHtml.match(/<audio/g) || []).length, 1);
{
    ...
    eq('the rendered src decodes to the topic 13 recording',
        decodeURIComponent(src).replace(/^\.\.\//, ''), 'audios/Б2 14 урок.mp3');
...
console.log(`  UI CSS ${uiCss.length}b · page CSS ${pageCss.length}b · session CSS ${sessionCss.length}b`
    + ` · chips ${CHIPS} · inputs ${INPUTS} · tables ${(t14.grammar.match(/class="b2g-t"/g) || []).length}`
...
<div class="vp-grammar">${t14.grammar}</div>
${stepsHtml}
<div class="uz-foot">
...
VIEWPORTS.forEach(([w, h]) => {
    ...
    /* Правда / Ложь chips */
    eq(`${label}: every option chip rendered`, r.chips, 90);
...
console.log('='.repeat(60));
if (fail) {
    console.log(`  ❌ B2 TOPIC 14 VIEWPORT: ${fail} failed / ${pass + fail}\n`);
    failures.forEach((f, i) => console.log(`   ${i + 1}. ${f}`));
    console.log('='.repeat(60) + '\n');
    process.exit(1);
}
console.log(`  ✅ B2 TOPIC 14 VIEWPORT: ${pass}/${pass} passed (measured in Chrome)`);
console.log('='.repeat(60) + '\n');
