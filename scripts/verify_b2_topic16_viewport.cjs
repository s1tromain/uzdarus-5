#!/usr/bin/env node
/**
 * verify_b2_topic16_viewport.cjs — Topic 16 must fit a phone, measured by a real
 * browser.
 *
...
const t16 = jw.B2_LESSON_DATA.topics.find((t) => t.id === 16);
ok(!!t16, 'topic 16 lesson data loaded');
if (!t16) { console.log('missing lesson 16'); process.exit(1); }
const groups = t16.exercises;
...
eq('every option chip rendered (ex1 2×10 + ex10 8×10 + audio 2×10)', CHIPS, 120);
eq('all eighty writing inputs rendered (eight open input groups)', INPUTS, 80);
eq('the audio player rendered exactly once', (stepsHtml.match(/<audio/g) || []).length, 1);
{
    ...
    eq('the rendered src decodes to the topic 13 recording',
        decodeURIComponent(src).replace(/^\.\.\//, ''), 'audios/Б2 16 урок.mp3');
...
console.log(`  UI CSS ${uiCss.length}b · page CSS ${pageCss.length}b · session CSS ${sessionCss.length}b`
    + ` · chips ${CHIPS} · inputs ${INPUTS} · tables ${(t16.grammar.match(/class="b2g-t"/g) || []).length}`
...
<div class="vp-grammar">${t16.grammar}</div>
${stepsHtml}
<div class="uz-foot">
...
VIEWPORTS.forEach(([w, h]) => {
    ...
    /* Правда / Ложь chips */
    eq(`${label}: every option chip rendered`, r.chips, 120);
    ...
    eq(`${label}: all eighty writing inputs rendered`, r.inputs, 80);
...
console.log('='.repeat(60));
if (fail) {
    console.log(`  ❌ B2 TOPIC 16 VIEWPORT: ${fail} failed / ${pass + fail}\n`);
    failures.forEach((f, i) => console.log(`   ${i + 1}. ${f}`));
    console.log('='.repeat(60) + '\n');
    process.exit(1);
}
console.log(`  ✅ B2 TOPIC 16 VIEWPORT: ${pass}/${pass} passed (measured in Chrome)`);
console.log('='.repeat(60) + '\n');
