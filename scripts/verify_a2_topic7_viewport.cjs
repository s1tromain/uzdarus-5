...
const t6 = vm.runInNewContext('(' + S.slice(j, e + 1) + ')', {}).topics.find((t) => t.id === 7);
const groups = t6.topic7Exercises.exercises;
      ...
      [360, 390].forEach(function (px) {
        ...
        wrap.querySelectorAll('*').forEach(function (n) {
          if (Math.round(n.getBoundingClientRect().width) > px + 1) {
            ...
            over.push((n.tagName + '.' + (n.className || '')).slice(0, 50));
          }
        });
...
const tmp = path.join(os.tmpdir(), 'uz-t7-viewport.html');
...
console.log('='.repeat(60));
if (fail) {
    console.log(`  ❌ A2 TOPIC 7 VIEWPORT: ${fail} failed / ${pass + fail}\n`);
    failures.forEach((f, i2) => console.log(`   ${i2 + 1}. ${f}`));
    console.log('='.repeat(60) + '\n');
    process.exit(1);
}
console.log(`  ✅ A2 TOPIC 7 VIEWPORT: ${pass}/${pass} passed (measured in Chrome)`);
console.log('='.repeat(60) + '\n');
