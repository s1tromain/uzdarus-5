#!/usr/bin/env node
/**
 * verify_b2_topic7_viewport.cjs — Topic 7 must fit a phone, measured by a real
 * browser.
 *
 * jsdom has no layout engine, so verify_b2_layout.cjs can only make claims
 * about CSS SOURCE. This renders the ACTUAL Topic 7 markup with the ACTUAL
 * stylesheets in headless Chrome and reads back real geometry.
 *
 * What this lesson puts under pressure:
 *   - twenty-seven two-column grammar tables carrying long Russian sentences,
 *   - tip/warning cards with a 52px icon gutter,
 *   - SEVENTY writing inputs, forty of them for whole-sentence production,
 *   - the long open prompts of Ex10 (up to ~90 characters, no gap to break at),
...
const t7 = jw.B2_LESSON_DATA.topics.find((t) => t.id === 7);
ok(!!t7, 'topic 7 lesson data loaded');
if (!t7) { console.log('missing lesson 7'); process.exit(1); }
const groups = t7.exercises;
...
console.log(`  UI CSS ${uiCss.length}b · page CSS ${pageCss.length}b · session CSS ${sessionCss.length}b`
    + ` · chips ${CHIPS} · inputs ${INPUTS} · tables ${(t7.grammar.match(/class="b2g-t"/g) || []).length}`
...
<div class="vp-grammar">${t7.grammar}</div>
${stepsHtml}
<div class="uz-foot">
...
const tmp = path.join(os.tmpdir(), 'uz-b2t7-viewport.html');
...
VIEWPORTS.forEach(([w, h]) => {
    ...
    /* grammar tables */
    eq(`${label}: all twenty-seven grammar tables rendered`, r.tables, 27);
    ...
    eq(`${label}: both tip/warning cards rendered`, r.cards, 4);
...
console.log('='.repeat(60));
if (fail) {
    console.log(`  ❌ B2 TOPIC 7 VIEWPORT: ${fail} failed / ${pass + fail}\n`);
    failures.forEach((f, i) => console.log(`   ${i + 1}. ${f}`));
    console.log('='.repeat(60) + '\n');
    process.exit(1);
}
console.log(`  ✅ B2 TOPIC 7 VIEWPORT: ${pass}/${pass} passed (measured in Chrome)`);
console.log('='.repeat(60) + '\n');
