#!/usr/bin/env node
/**
 * verify_b2_final_exam_viewport.cjs — the B2 final exam must fit a phone,
 ...
 *
 * B2 is the platform's last course, so this is the last screen between a
 * learner and a certificate. It is also the widest exam paper on the site —
 * ten groups instead of five, and options that are whole sentences rather
 * than single words — which is exactly the shape that overflows a phone.
 *
...
(async () => {
    ...
    const tmp = path.join(os.tmpdir(), 'uz-b2exam-viewport.html');
...
<\/script></body></html>`;
        ...
        for (const [w, h] of VIEWPORTS) {
            ...
            /* TOUCH TARGETS — measured, and asserted at the size this page
               ...
               The ANSWER CHIPS are what a learner taps 63 times, so they carry
               ...
               stylesheet is live in a1-, a2- and b1-final-exam.html.
               Raising them to 44px would be a redesign of four shipping exam
               pages, which this task is explicitly not doing.
               ...
               PRE-EXISTING and SHARED with A1, A2 and B1 — it is reported, not
               silently absorbed, and not fixed by unilaterally restyling the
               three exam pages that already ship it. */
            if (r.chipMin != null) {
                ok(r.chipMin >= 36,
    ...
    console.log('='.repeat(60));
    if (fail) {
        console.log(`  ❌ B2 FINAL EXAM VIEWPORT: ${fail} failed / ${pass + fail}\n`);
        failures.slice(0, 20).forEach((f, i) => console.log(`   ${i + 1}. ${f}`));
        console.log('='.repeat(60) + '\n');
        process.exit(1);
    }
    console.log(`  ✅ B2 FINAL EXAM VIEWPORT: ${pass}/${pass} passed (measured in Chrome)`);
    console.log('='.repeat(60) + '\n');
