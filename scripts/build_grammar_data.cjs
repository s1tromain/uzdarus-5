...
/**
 * ONE ADDRESS FOR EVERY PICTURE, VALID FROM EVERY PAGE.
 *
 * The material was written inside /paid-courses/, so its images say
 * "../images/a1/…". That resolves from a page one level down and from nowhere
 * else — the public reader sits at the site root, where the same path climbs
 * above it and 404s. And several of the files are named in Cyrillic with
 * spaces, which a src attribute has to carry percent-encoded.
 *
 * So every local path is rewritten once, here, to a root-absolute encoded URL:
 * "/images/a1/Grammatik%20qism%206%20dars.jpg". Absolute http(s) URLs are left
 * exactly as they are.
 */
function siteAbsolute(src) {
    var raw = String(src || '').trim();
    if (!raw || /^(https?:|data:|\/\/)/i.test(raw)) return raw;
    var clean = decodeURI(raw.replace(/^(\.\.\/)+/, '').replace(/^\.?\//, ''));
    /* a path that still tries to climb is not a path we will serve */
    if (clean.indexOf('../') >= 0) return raw;
    return '/' + clean.split('/').map(encodeURIComponent).join('/');
}

/** Rewrite every img src in the material and return the addresses used. */
function normaliseImages(html) {
    var used = [];
    var out = String(html || '').replace(
        /(<img\b[^>]*?\bsrc=)(["'])([^"']+)\2/gi,
        function (all, head, q, src) {
            var abs = siteAbsolute(src);
            used.push(abs);
            return head + q + abs + q;
        });
    return { html: out, images: used };
}
...
try {
    ...
    for (const [course, url] of Object.entries(PAGES)) {
        ...
        const raw = await p.evaluate(`
            ...
            /* THE OLD ZOOM LABEL IS A LEFTOVER CONTROL. It told the learner to
               press the picture, which worked through an onclick the build
               strips; the reader gives every picture its own button, so the
               label would sit under it saying the same thing twice. */
            function withoutStaleControls(html) {
                var d = document.createElement('div');
                d.innerHTML = String(html || '');
                Array.prototype.slice.call(d.querySelectorAll('.zoom-hint'))
                    .forEach(function (el) { el.parentElement && el.parentElement.removeChild(el); });
                return d.innerHTML;
            }
            ...
            return { id: t.id, title: String(t.title||''),
                     ...
                     grammar: withoutStaleControls(t.grammar),
                     explanation: String((t.explanation && t.explanation.uz) || ''),
                     content: withoutStaleControls(withoutDeckPromo(t.content)) }; }));`);
        const topics = JSON.parse(raw).map((t) => {
            ...
            const explain = t.explanation.trim()
                ...
                : '';
            const joined = [t.grammar, explain, t.content]
                .map((x) => String(x || '').trim()).filter(Boolean).join('\n')
                .replace(/\son[a-z]+\s*=\s*"[^"]*"/gi, '')
                .replace(/\son[a-z]+\s*=\s*'[^']*'/gi, '')
                .trim();
            const fixed = normaliseImages(joined);
            const body = fixed.html;
            const frame = (frames[course] || {})[String(t.id)] || {};
            totalTopics++;
            if (textOf(body).length >= 120) withText++;
            const images = fixed.images.filter((x) => /\.(jpg|jpeg|png|webp|gif)$/i.test(x));
