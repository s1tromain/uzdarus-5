#!/usr/bin/env node
/**
 * verify_b2_topic11_viewport.cjs — Topic 11 must fit a phone, measured by a real
 * browser.
 *
 * jsdom has no layout engine, so verify_b2_layout.cjs can only make claims
 * about CSS SOURCE. This renders the ACTUAL Topic 11 markup with the ACTUAL
...
const t11 = jw.B2_LESSON_DATA.topics.find((t) => t.id === 11);
ok(!!t11, 'topic 11 lesson data loaded');
if (!t11) { console.log('missing lesson 7'); process.exit(1); }
const groups = t11.exercises;
...
eq('every option chip rendered (ex3 3×10 + audio 2×10)', CHIPS, 50);
...
console.log(`  UI CSS ${uiCss.length}b · page CSS ${pageCss.length}b · session CSS ${sessionCss.length}b`
    + ` · chips ${CHIPS} · inputs ${INPUTS} · tables ${(t11.grammar.match(/class="b2g-t"/g) || []).length}`
...
<div class="vp-grammar">${t11.grammar}</div>
${stepsHtml}
<div class="uz-foot">
...
const tmp = path.join(os.tmpdir(), 'uz-b2t11-viewport.html');
...
VIEWPORTS.forEach(([w, h]) => {
    ...
    /* grammar tables */
    eq(`${label}: all fifteen grammar tables rendered`, r.tables, 15);
    ...
    eq(`${label}: all six tip/warning cards rendered`, r.cards, 6);
...
console.log('='.repeat(60));
if (fail) {
    console.log(`  ❌ B2 TOPIC 11 VIEWPORT: ${fail} failed / ${pass + fail}\n`);
    failures.forEach((f, i) => console.log(`   ${i + 1}. ${f}`));
    console.log('='.repeat(60) + '\n');
    process.exit(1);
}
console.log(`  ✅ B2 TOPIC 11 VIEWPORT: ${pass}/${pass} passed (measured in Chrome)`);
console.log('='.repeat(60) + '\n');
