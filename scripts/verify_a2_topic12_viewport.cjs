...
const t6 = vm.runInNewContext('(' + S.slice(j, e + 1) + ')', {}).topics.find((t) => t.id === 12);
const groups = t6.topic12Exercises.exercises;
...
const tmp = path.join(os.tmpdir(), 'uz-t12-viewport.html');
...
console.log('='.repeat(60));
if (fail) {
    console.log(`  ❌ A2 TOPIC 12 VIEWPORT: ${fail} failed / ${pass + fail}\n`);
    failures.forEach((f, i2) => console.log(`   ${i2 + 1}. ${f}`));
    console.log('='.repeat(60) + '\n');
    process.exit(1);
}
console.log(`  ✅ A2 TOPIC 12 VIEWPORT: ${pass}/${pass} passed (measured in Chrome)`);
console.log('='.repeat(60) + '\n');
