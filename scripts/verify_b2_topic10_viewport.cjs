#!/usr/bin/env node
/**
 * verify_b2_topic10_viewport.cjs — Topic 10 must fit a phone, measured by a real
 * browser.
 *
 * jsdom has no layout engine, so verify_b2_layout.cjs can only make claims
 * about CSS SOURCE. This renders the ACTUAL Topic 10 markup with the ACTUAL
 ...
 *   - ONE HUNDRED writing inputs, all of them taking a full sentence,
...
const t10 = jw.B2_LESSON_DATA.topics.find((t) => t.id === 10);
ok(!!t10, 'topic 10 lesson data loaded');
if (!t10) { console.log('missing lesson 7'); process.exit(1); }
const groups = t10.exercises;
...
console.log(`  UI CSS ${uiCss.length}b · page CSS ${pageCss.length}b · session CSS ${sessionCss.length}b`
    + ` · chips ${CHIPS} · inputs ${INPUTS} · tables ${(t10.grammar.match(/class="b2g-t"/g) || []).length}`
...
<div class="vp-grammar">${t10.grammar}</div>
${stepsHtml}
<div class="uz-foot">
...
const tmp = path.join(os.tmpdir(), 'uz-b2t10-viewport.html');
...
VIEWPORTS.forEach(([w, h]) => {
    ...
    /* grammar tables */
    eq(`${label}: all twenty-one grammar tables rendered`, r.tables, 21);
    ...
    eq(`${label}: all three tip/warning cards rendered`, r.cards, 3);
...
console.log('='.repeat(60));
if (fail) {
    console.log(`  ❌ B2 TOPIC 10 VIEWPORT: ${fail} failed / ${pass + fail}\n`);
    failures.forEach((f, i) => console.log(`   ${i + 1}. ${f}`));
    console.log('='.repeat(60) + '\n');
    process.exit(1);
}
console.log(`  ✅ B2 TOPIC 10 VIEWPORT: ${pass}/${pass} passed (measured in Chrome)`);
console.log('='.repeat(60) + '\n');
