#!/usr/bin/env node
/**
 * verify_b2_topic9_viewport.cjs — Topic 9 must fit a phone, measured by a real
 * browser.
 *
 * jsdom has no layout engine, so verify_b2_layout.cjs can only make claims
 * about CSS SOURCE. This renders the ACTUAL Topic 9 markup with the ACTUAL
 * stylesheets in headless Chrome and reads back real geometry.
 *
 * What this lesson puts under pressure:
 *   - fourteen two-column grammar tables carrying long Russian sentences,
 *   - the warning and tip cards with their 52px icon gutters,
 *   - ONE HUNDRED writing inputs, twenty of them for open production,
 *   - the two-line dialogue prompts of Ex8 and the two-line Ex10 prompts,
 *   - the long prompts of Ex1, which carry a «(вариант / вариант)» hint inline.
 * This lesson has NO audio and NO option chips, so neither is measured; the
 * static suite asserts their absence instead.
 *
...
const t9 = jw.B2_LESSON_DATA.topics.find((t) => t.id === 9);
ok(!!t9, 'topic 9 lesson data loaded');
if (!t9) { console.log('missing lesson 7'); process.exit(1); }
const groups = t9.exercises;
...
ok(/\.b2g-tone-sv\b/.test(uiCss) && /\.b2g-tone-nsv\b/.test(uiCss),
    'the measured CSS styles the aspect-colour spans this lesson uses');
...
eq('all one hundred writing inputs rendered (80 deterministic + 20 open)', INPUTS, 100);
eq('this lesson renders no option chips', (stepsHtml.match(/class="b2h-opt"/g) || []).length, 0);
ok(!/<audio/.test(stepsHtml), 'this lesson renders no audio player — the source supplied none');
...
console.log(`  UI CSS ${uiCss.length}b · page CSS ${pageCss.length}b · session CSS ${sessionCss.length}b`
    + ` · inputs ${INPUTS} · tables ${(t9.grammar.match(/class="b2g-t"/g) || []).length}`
...
<div class="vp-grammar">${t9.grammar}</div>
${stepsHtml}
<div class="uz-foot">
...
const tmp = path.join(os.tmpdir(), 'uz-b2t9-viewport.html');
...
VIEWPORTS.forEach(([w, h]) => {
    ...
    console.log(`  ${label.padEnd(9)} client=${r.clientWidth} scroll=${r.scrollWidth} `
        + `overflow=${r.overflow} · inputs ${r.minInputW}-${r.maxInputW}px `
        + `· tables ${r.tables} (${r.tablesScrollable} scrollable, `
    ...
    /* grammar tables */
    eq(`${label}: all fourteen grammar tables rendered`, r.tables, 14);
    ...
    eq(`${label}: both the warning and the tip card rendered`, r.cards, 2);
    ok(r.maxCardRight <= r.clientWidth + 1,
        `${label}: no tip/warning card runs past the right edge (${r.maxCardRight} <= ${r.clientWidth})`);

    eq(`${label}: no option chip is on the page`, r.chips, 0);

    /* seventy writing inputs — forty of them take a whole sentence */
    eq(`${label}: all one hundred writing inputs rendered`, r.inputs, 100);
    ...
    eq(`${label}: no audio player is on the page`, r.audioW, 0);
    ...
    if (w === 500 && r.narrow) {
        NARROW.forEach((px) => {
            ...
            console.log(`  ${String(px + 'px container').padEnd(14)} `
                ...
                + ` · narrowest input ${n.minInputW}px`);
...
console.log('='.repeat(60));
if (fail) {
    console.log(`  ❌ B2 TOPIC 9 VIEWPORT: ${fail} failed / ${pass + fail}\n`);
    failures.forEach((f, i) => console.log(`   ${i + 1}. ${f}`));
    console.log('='.repeat(60) + '\n');
    process.exit(1);
}
console.log(`  ✅ B2 TOPIC 9 VIEWPORT: ${pass}/${pass} passed (measured in Chrome)`);
console.log('='.repeat(60) + '\n');
