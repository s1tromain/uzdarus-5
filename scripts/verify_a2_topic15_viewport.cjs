#!/usr/bin/env node
/**
 * verify_a2_topic15_viewport.cjs — Topic 15 must fit a phone, measured by a real
 * browser.
 *
...
const t6 = vm.runInNewContext('(' + S.slice(j, e + 1) + ')', {}).topics.find((t) => t.id === 15);
const groups = t6.topic15Exercises.exercises;
    ...
    ok(stepsHtml.includes(cls), `the rendered builder markup contains .${cls}`));
{
    ...
    const want = ex9.items.reduce((n, it) => n + it.q.split('/').length, 0);
    const got = (html.match(/class="uzb-tok"/g) || []).length;
    ok(got === want, `ex9 renders one card per cue word (${got} of ${want})`);
    /* «каждый день» is glued: it must be ONE card, not two */
    ok(/>каждый день</.test(html),
        '«каждый день» is rendered as a single glued card, as the source cue writes it');
}
  ...
  var out = {
    ...
    builder: (function () {
      ...
      return {
        ...
        gluedW: (function () {
          var g = toks.filter(function (t) { return (t.textContent || '').trim() === 'каждый день'; });
          return g.length ? Math.round(g[0].getBoundingClientRect().width) : 0;
        })(),
        gluedCards: toks.filter(function (t) {
          return (t.textContent || '').trim() === 'каждый день';
        }).length
      };
    })()
  };
...
const tmp = path.join(os.tmpdir(), 'uz-t15-viewport.html');
...
VIEWPORTS.forEach(([w, h]) => {
    ...
    if (b) {
        ...
        eq(`${label}: all 42 word cards rendered`, b.tokens, 42);
        ...
        /* «каждый день» is glued into ONE card by design; it is also the widest
           card in the lesson, so it is the one that would overflow a phone. */
        eq(`${label}: «каждый день» stays a single glued card`, b.gluedCards, 1);
        ok(b.gluedW > 0 && b.gluedW <= r.clientWidth,
            `${label}: the glued «каждый день» card fits (${b.gluedW}px)`);
    }
...
console.log('='.repeat(60));
if (fail) {
    console.log(`  ❌ A2 TOPIC 15 VIEWPORT: ${fail} failed / ${pass + fail}\n`);
    failures.forEach((f, i2) => console.log(`   ${i2 + 1}. ${f}`));
    console.log('='.repeat(60) + '\n');
    process.exit(1);
}
console.log(`  ✅ A2 TOPIC 15 VIEWPORT: ${pass}/${pass} passed (measured in Chrome)`);
console.log('='.repeat(60) + '\n');
