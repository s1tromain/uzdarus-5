#!/usr/bin/env node
/**
 * verify_b1_exercise_viewport.cjs — the B1 exercise section on real screens.
 *
 * B1 now has faces that never existed when the other viewport suites were
 ...
 * ================================================================ */
/* structurally different families: T1 has the small 5- and 4-item exercises
   and a listening block, T8 a five-item group, T14 the ten-exercise lesson,
   T20 the widest case — eleven exercises, a reading passage and open prompts. */
const TOPICS = [1, 8, 14, 20];
const captured = {};
let uiCss = '', sessionCss = '';

function newCtx(extra) { return H.makePage(extra || {}); }
function groupsOf(ctx, id) {
    return ctx.window.B1Host.groupsOf(ctx.courseData.topics.find((t) => t.id === id));
}
...
TOPICS.forEach((id) => {
    ...
    /* --- a FAILED attempt: the sub-80% verdict and Qayta topshirish --- */
    {
        ...
        d.querySelectorAll('.b2h-item').forEach((node, i) => {
            ...
            /* AGAINST EVERY ACCEPTED ANSWER, not just the first. Several B1
               exercises accept a list, so "anything other than answer[0]" can
               still be right — topic 14 scored 8/10 that way and the retry
               state this fragment exists to measure never appeared. */
            const accepted = item
                ? (Array.isArray(item.answer) ? item.answer : [item.answer])
                    .map((a) => String(a == null ? '' : a).trim().toLowerCase())
                : [];
            const opts = [...node.querySelectorAll('.b2h-opt')];
            /* data-value, NOT textContent: a group with style "test" renders a
               letter key inside the chip, so its text reads "Aбуду" and never
               matches any answer — which made every "wrong" pick land on the
               first option, and that one is often the right one. */
            const valueOf = (o) => String(o.getAttribute('data-value') != null
                ? o.getAttribute('data-value') : o.textContent).trim().toLowerCase();
            const wrong = opts.find((o) => accepted.indexOf(valueOf(o)) < 0);
    ...
    /* --- the resume dialog, raised by a genuine stale draft --- */
    {
        ...
        const groups = groupsOf(ctx, id);
        ctx.window.localStorage.setItem(B1.draftKey('u-1', id), JSON.stringify({
            v: 1, fingerprint: B1.fingerprint(groups, id), course: 'B1', topicId: id,
...
for (const id of TOPICS) {
    ...
    ok(/b1-save-error/.test(states.saveerr), `T${id}: the save-error panel rendered`);
    ...
    const snapshot = base.window.B1Host.buildSnapshot(id, groups, H.finishedAttempt(groups, 2));

    const doneCtx = newCtx({ userQuizResults: { ['topic_' + id]: { b1ExerciseResult: snapshot } },
                             courseState: { topicComponents: { [id]: { exercisesCompleted: true } } } });
    doneCtx.window.UzExerciseUI.injectStyles();
    doneCtx.mountB1Practice(id);
    ...
    ok(!!doneCtx.q('.b1-review'), `T${id}: the review rendered`);
    states.review = doneCtx.quizSection.innerHTML;

    const syncCtx = newCtx({ userQuizResults: { ['topic_' + id]: { b1ExerciseResult: snapshot } } });
    syncCtx.mountB1Practice(id);
...
['.b1-done-panel', '.b1-review', '.b1-cta', '.b1-sync-warn', '.b1-save-error']
...
const CRITICAL = {
    ...
    saveerr: '.b1-retry-save', done: '.b1-cta', sync: '.b1-retry-save',
    review: '.b1-review-close', normal: '.uz-foot button',
...
const tmp = path.join(os.tmpdir(), 'uz-b1-viewport.html');
const worst = {};
...
console.log('='.repeat(60));
if (fail) {
    console.log(`  ❌ B1 EXERCISE VIEWPORT: ${fail} failed / ${pass + fail}\n`);
    failures.slice(0, 25).forEach((f, i) => console.log(`   ${i + 1}. ${f}`));
    console.log('='.repeat(60) + '\n');
    process.exit(1);
}
console.log(`  ✅ B1 EXERCISE VIEWPORT: ${pass}/${pass} passed`);
console.log('='.repeat(60) + '\n');
