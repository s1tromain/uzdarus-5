#!/usr/bin/env node
/**
 * verify_b2_topic12_viewport.cjs — Topic 12 must fit a phone, measured by a real
 * browser.
 *
...
const t12 = jw.B2_LESSON_DATA.topics.find((t) => t.id === 12);
ok(!!t12, 'topic 12 lesson data loaded');
if (!t12) { console.log('missing lesson 7'); process.exit(1); }
const groups = t12.exercises;
...
eq('every option chip rendered (ex6 2×10 + audio 2×10)', CHIPS, 40);
...
console.log(`  UI CSS ${uiCss.length}b · page CSS ${pageCss.length}b · session CSS ${sessionCss.length}b`
    + ` · chips ${CHIPS} · inputs ${INPUTS} · tables ${(t12.grammar.match(/class="b2g-t"/g) || []).length}`
...
<div class="vp-grammar">${t12.grammar}</div>
${stepsHtml}
<div class="uz-foot">
...
console.log('='.repeat(60));
if (fail) {
    console.log(`  ❌ B2 TOPIC 12 VIEWPORT: ${fail} failed / ${pass + fail}\n`);
    failures.forEach((f, i) => console.log(`   ${i + 1}. ${f}`));
    console.log('='.repeat(60) + '\n');
    process.exit(1);
}
console.log(`  ✅ B2 TOPIC 12 VIEWPORT: ${pass}/${pass} passed (measured in Chrome)`);
console.log('='.repeat(60) + '\n');
