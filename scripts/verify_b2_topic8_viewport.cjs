#!/usr/bin/env node
/**
 * verify_b2_topic8_viewport.cjs — Topic 8 must fit a phone, measured by a real
 * browser.
 *
 * jsdom has no layout engine, so verify_b2_layout.cjs can only make claims
 * about CSS SOURCE. This renders the ACTUAL Topic 8 markup with the ACTUAL
 * stylesheets in headless Chrome and reads back real geometry.
 *
 * What this lesson puts under pressure:
 *   - twenty-two two-column grammar tables carrying long Russian sentences,
 *   - the aspect-pair warning card with its 52px icon gutter,
 *   - ONE HUNDRED writing inputs, thirty of them for open production,
 *   - the long prompts of Ex1/Ex3, which carry a «(verb / verb)» hint inline,
...
const t8 = jw.B2_LESSON_DATA.topics.find((t) => t.id === 8);
ok(!!t8, 'topic 8 lesson data loaded');
if (!t8) { console.log('missing lesson 7'); process.exit(1); }
const groups = t8.exercises;
...
eq('every Правда/Ложь chip rendered', CHIPS, 20);
eq('all one hundred writing inputs rendered (60 deterministic + 40 open)', INPUTS, 100);
...
console.log(`  UI CSS ${uiCss.length}b · page CSS ${pageCss.length}b · session CSS ${sessionCss.length}b`
    + ` · chips ${CHIPS} · inputs ${INPUTS} · tables ${(t8.grammar.match(/class="b2g-t"/g) || []).length}`
...
<div class="vp-grammar">${t8.grammar}</div>
${stepsHtml}
<div class="uz-foot">
...
const tmp = path.join(os.tmpdir(), 'uz-b2t8-viewport.html');
...
VIEWPORTS.forEach(([w, h]) => {
    ...
    /* grammar tables */
    eq(`${label}: all twenty-two grammar tables rendered`, r.tables, 22);
...
console.log('='.repeat(60));
if (fail) {
    console.log(`  ❌ B2 TOPIC 8 VIEWPORT: ${fail} failed / ${pass + fail}\n`);
    failures.forEach((f, i) => console.log(`   ${i + 1}. ${f}`));
    console.log('='.repeat(60) + '\n');
    process.exit(1);
}
console.log(`  ✅ B2 TOPIC 8 VIEWPORT: ${pass}/${pass} passed (measured in Chrome)`);
console.log('='.repeat(60) + '\n');
